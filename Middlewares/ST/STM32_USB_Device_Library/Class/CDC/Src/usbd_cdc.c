...
#define USB_CDC_CONFIG_DESC_SIZ                     (67U - 0)
...
#else
#if SUPPORT_MUITL_EP
#define WINUSB_CONFIG_DESC_SIZE 32 + 14
#else
#define WINUSB_CONFIG_DESC_SIZE 32 //+ 14
#endif
...
#if SUPPORT_MUITL_EP
    /* Open EP IN */
    USBD_LL_OpenEP(pdev, CDC_IN_EP_2, USBD_EP_TYPE_BULK,
                   CDC_DATA_FS_IN_PACKET_SIZE);

    pdev->ep_in[CDC_IN_EP_2 & 0xFU].is_used = 1U;

    /* Open EP OUT */
    USBD_LL_OpenEP(pdev, CDC_OUT_EP_2, USBD_EP_TYPE_BULK,
                   CDC_DATA_FS_OUT_PACKET_SIZE);

    pdev->ep_out[CDC_OUT_EP_2 & 0xFU].is_used = 1U;
#endif
  }
  /* Open Command IN EP */
...
#if SUPPORT_MUITL_EP
      /* Prepare Out endpoint to receive next packet */
      USBD_LL_PrepareReceive(pdev, CDC_OUT_EP_2, hcdc->RxBuffer,
                             CDC_DATA_FS_OUT_PACKET_SIZE);
#endif
    }
  }
                              ...
                              USBD_SetupReqTypedef *req)
{
  ...
  printf("USBD_CDC_Setup:[0x%02X, 0x%02X, 0x%04X, 0x%04X, 0x%04X]\r\n", pdev->request.bmRequest, pdev->request.bRequest,pdev->request.wValue, pdev->request.wIndex,  pdev->request.wLength);
  ...
  case USB_REQ_TYPE_STANDARD:
	...
	//[bmRequest:0xC1, bRequest:0xA0, wValue:0x0000, wIndex:0x0005, wLength:0x000A]	
	if(req->bmRequest & 0x80) //设备到主机 (Device to Host)。
	{
	...
	{	
		if (req->wLength)
		{
			if (req->bmRequest & 0x80U)
			{
				...
				printf("len:%d:%s",hcdc->RxLength, (uint8_t *)(void *)hcdc->RxBuffer);
				USBD_CtlSendData(pdev, (uint8_t*)dat, strlen(dat));
			}
			else{
				printf("%s %d\r\n", __FUNCTION__, __LINE__);
				hcdc->CmdOpCode = req->bRequest;
...
static uint8_t *USBD_CDC_GetOtherSpeedCfgDesc(uint16_t *length)
{
  printf("USBD_CDC_GetOtherSpeedCfgDesc\n");	
  *length = sizeof(USBD_CDC_CfgFSDesc);
  return USBD_CDC_CfgFSDesc;
}
...
static void USBD_WinUSBGetDescriptor(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
	uint16_t len;
	uint8_t *pbuf;
	printf("USBD_WinUSBGetDescriptor\n");
	switch (req->wIndex)
	{ 
		case 0x04: // compat ID
